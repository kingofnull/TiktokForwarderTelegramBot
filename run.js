...
const {sendToChannel} = require('./com');
...
(async () => {
    try {
        ...
        console.log(posts);
        for(let p of posts.collector){
            if (!p.videoUrl) {
                return;
            }

            //console.log(p)
            let count=await BroadcastLog.count({where: {id: p.id}});
            if(count>0){
                console.log(`Already exists: ${p.id}`);
            }else{
                console.log(`Sending: ${p.id}`);
                await sendToChannel(p,BroadcastLog);
            }
